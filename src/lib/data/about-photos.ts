...
export type AboutPhoto = {
	...
	/**
	 * Intrinsic pixel size, needed **only** by the `tall` photos and only for layout stability.
	 *
	 * Every other card is `aspect-ratio: 1 / 1` with `height: 100%`, so its box is reserved by CSS
	 * before the file arrives and `width`/`height` attributes would be inert. The `tall` variant is
	 * `aspect-ratio: auto` + `height: auto` — its height comes from the image itself, so until the
	 * bytes land the card is zero-height and everything below it moves when they do.
	 *
	 * Measured CLS on `/about` is currently 0 because Chrome starts lazy loads ~1250px before they
	 * scroll into view, which is enough lead time on a fast connection to land the image while the
	 * card is still off-screen. That margin is a fixed distance, not a fixed duration, so it buys
	 * less and less as the connection gets slower — which is exactly the phone case. These three
	 * numbers make the reservation unconditional instead of a race the network usually wins.
	 */
	width?: number;
	height?: number;
};
...
export const aboutPhotos: AboutPhoto[] = [
	...
	{ src: '/about/IMG_0984.jpeg', fit: 'contain', tall: true, width: 640, height: 480 },
	{
	...
	{ src: '/about/IMG_7163.jpg', position: '50% 5%', padding: { left: 45, right: 45 } },
